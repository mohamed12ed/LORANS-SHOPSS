(function () {
  ...
  const PROMO_OFFER_ROTATION = [
    ...
    { titleKey: "gta.title", priceDa: "1600" },
  ];
  ...
  platformChips.forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      if (activePlatform === v) {
        ...
        activePlatform = v;
        platformChips.forEach((c) => {
          const on = c.getAttribute("data-platform-filter") === activePlatform;
  ...
  document.querySelectorAll(".product-card, .payment-card, .step").forEach((el) => {
    el.style.opacity = "0";
  ...
  document.querySelectorAll(".btn-buy-primary").forEach((btn) => {
    btn.addEventListener("click", () => {
      const t = btn.textContent;
      btn.textContent = "✓ تم!";
      btn.style.background = "linear-gradient(180deg, #16a34a, #15803d)";
      setTimeout(() => {
        btn.textContent = t || "اطلب الآن";
